# Cluster 업그레이드 절차 정리 (Governanced by policies)
...
$ CLUSTER=local-cluster
$ oc -n $CLUSTER wait --timeout=2h --for=jsonpath='{.status.compliant}'=Compliant policy policies.cv-inf-check
...
### Change sub-approval label
```
## Change managecluster's sub-approval label
$ CLUSTER=local-cluster
